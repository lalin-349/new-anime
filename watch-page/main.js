...
document.querySelector(".p5 p").addEventListener("click",()=> { playFive();
    ...
    const playSix = () =>{
        ...
        gb.load();
        gb.play();
        const pic = document.querySelector(".banner-image").src = "../img2/wind.jpg";
        const title = document.querySelector(".title").textContent = "Wind Breaker";
        const description = "The anime Wind Breaker follows the story of Haruka Sakura, a skilled fighter who enrolls at Furin High School, a place known for valuing strength over academics. Initially focused on climbing to the top, Haruka discovers the school's true mission: protecting their town from harm. Along the way, he learns the value of teamwork and fighting for others, rather than just himself. It's an action-packed series with themes of camaraderie and personal growth.";
        const des = document.querySelector(".des").textContent = description;
    }
    ...
    const playSeven = () =>{
        ...
        gb.load();
        gb.play();
        const pic = document.querySelector(".banner-image").src = "../img2/classe.jpg";
        const title = document.querySelector(".title").textContent = "Classroom of the elite";
        const description = "**Classroom of the Elite** is a psychological thriller anime based on the light novel series by Shōgo Kinugasa. The story revolves around Kiyotaka Ayanokoji, a seemingly ordinary student at Tokyo Koudo Ikusei Senior High School, a prestigious institution where students are ranked based on merit. Kiyotaka finds himself in Class 1-D, which is considered the lowest tier, but he soon reveals his exceptional intellect and strategic thinking as he navigates the school's competitive and manipulative environment.";
        const des = document.querySelector(".des").textContent = description;
    }
    ...
    const playEight = () =>{
        ...
        gb.load();
        gb.play();
        const pic = document.querySelector(".banner-image").src = "../img2/summer.jpg";
        const title = document.querySelector(".title").textContent = "summertime render";
        const description = "**Summertime Render** (or *Summer Time Rendering*) is a gripping mystery and supernatural anime. The story follows Shinpei Ajiro, who returns to his hometown on Hitogashima Island for the funeral of his childhood friend, Ushio. However, strange occurrences and the discovery of mysterious entities called Shadows lead him to uncover dark secrets about the island. With time loops and suspenseful twists, the anime keeps viewers on the edge of their seats.";
        const des = document.querySelector(".des").textContent = description;
    }
...
const playThirteen= () =>{
            const gg = document.querySelector(".btn13");
            const gb = document.querySelector(".video-tag");
            gb.src = gg.getAttribute("src");
            gb.load();
            gb.play();
